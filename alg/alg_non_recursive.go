...
import (
	sf "symmetric_group/alg/safe_struct"
...
func CharacterTableSchur(n int8, file *os.File, multipleThread bool) { // just print it here
	...
	// var table = make(map[int32]map[int32]int64, p_size)
...
func PowerSumToSchur(partition []int8) map[int32]int64 {
	var acc = map[int32]int64{}
	acc[pt.PKey([]int8{})] = 1
	var i = len(partition) - 1
	for ; i >= 0; i-- {
		// fmt.Println(acc)
		var prod = map[int32]int64{}
...
func Support(comb map[int32]int64) map[int32]int64 {
